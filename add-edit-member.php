...
if (!empty($_POST)) {
  ...
  $createMemberStmt->execute(['first_name' => $first_name, 'last_name' => $last_name, 'address' => $address, 'tier_id' => $tier_id]);
        ...
        try {
            ...
            $updateMemberStmt = $db->prepare('UPDATE members SET first_name = :first_name, last_name = :last_name, address=:address WHERE tier_id = :tier_id'); ;
            // Execute it here
           $updateMemberStmt->execute(['first_name' => $first_name, 'last_name' => $last_name, 'address' => $address, 'tier_id' => $tier_id]);
            // We'll use the rowCount() function to check if a row was modified. If so, we'll assume the statement was successful, otherwise, it likely failed.
